            ...
            <div class="card shadow border-0">
                ...
                <div class="card-body">
                    <div class="col-sm-12" id="filter">
                        <form class="row" id="filter_contract" method="post" action="{{route('posting.filter_contracts')}}">
...
<script>
    ...
    var table = $('#contract_table').DataTable({
            ...
            , {
                ...
                , action: function(e, dt, node, config) {
                    ...
                    if (unique_status.length > 1) {
                        ...
                        let contract_ids = table.cells(rows, 1).data().toArray().toString();
                        $("#posting_form").prop('action', "{{route('posting.submit_contract')}}");
                        $("#action").val("{{route('posting.submit_contract')}}");
                        $("#id").val(contract_ids);
                        $("#download").val("");
                        $("#modal_form").modal('show');
                    }
        ...
        , columns: [{
                ...
                , render: function(data, type, row) {
                    ...
                    return '<div style="white-space: nowrap">' + status + '<a  class="btn btn-sm shadow-0 border-0 btn-primary text-white" target="_blank" data-toggle="tooltip" data-placement="top" title="View Attachment/s"  href="/archive/view_contract_attachments/' + row.contract_id + '"><i class="ni ni-tv-2"></i></a></div>';
    ...
    if ("{{session('message')}}") {
        if ("{{session('message')}}" == "missing_attachment") {
            ...
            if(download=="true"){
                 $("#posting_form").prop('action', "{{route('posting.download_contract_zip')}}");
                 $("#posting_form").submit();
