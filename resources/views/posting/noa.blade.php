            ...
            <div class="card shadow border-0">
                ...
                <div class="card-body">
                    <div class="col-sm-12" id="filter">
                        <form class="row" id="filter_notice_of_award" method="post" action="{{route('posting.filter_notice_of_awards')}}">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered" id="noa_table">
                            <thead class="">
...
<script>
    ...
    // datatables
    $('#noa_table thead tr').clone(true).appendTo('#noa_table thead');
    $('#noa_table thead tr:eq(1)').removeClass('bg-primary');
    var data = @json(session('data'));
    if (data == null) {
        data = @json($data);
    }
    var table = $('#noa_table').DataTable({
        dom: 'Bfrtip'
        , buttons: [{
                text: 'Hide Filter'
                , attr: {
            ...
            , {
                ...
                , action: function(e, dt, node, config) {
                    ...
                    if (unique_status.length > 1) {
                        ...
                        let notice_award_ids = table.cells(rows, 1).data().toArray().toString();
                        $("#posting_form").prop('action', "{{route('posting.submit_notice_of_award')}}");
                        $("#action").val("{{route('posting.submit_notice_of_award')}}");
                        $("#id").val(notice_award_ids);
                        $("#download").val("");
                        $("#modal_form").modal('show');
                    }
                }
        ...
        , columns: [{
                ...
                , render: function(data, type, row) {
                    ...
                    return '<div style="white-space: nowrap">' + status + '<a  class="btn btn-sm shadow-0 border-0 btn-primary text-white" target="_blank" data-toggle="tooltip" data-placement="top" title="View Attachment/s"  href="/archive/view_noa_attachments/' + row.notice_award_id + '"><i class="ni ni-tv-2"></i></a></div>';
    ...
    if ("{{session('message')}}") {
        if ("{{session('message')}}" == "missing_attachment") {
            ...
            if (download == "true") {
                $("#posting_form").prop('action', "{{route('posting.download_noa_zip')}}");
                $("#posting_form").submit();
    ...
    // events
    $('#noa_table thead tr:eq(1) th').each(function(i) {
        var title = $(this).text();
        if (title != "") {
