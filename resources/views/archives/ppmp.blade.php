      ...
      <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            action="{{route('archive.submit_ppmp')}}" enctype="multipart/form-data">
            @csrf
            <div class="row d-flex">
              ...
              <div class="form-group col-xs-12 col-sm-12 col-lg-11 mb-0">
                <label for="year">PPMP Year<span class="text-red">*</span></label>
              ...
              <div class="form-group col-xs-12 col-sm-12 col-lg-11 mb-0 d-none">
                <label for="ppmp_type">PPMP Type<span class="text-red">*</span></label>
                <input type="text" id="ppmp_type" name="ppmp_type" class="form-control form-control-sm" readonly value="{{$ppmp_type}}">
                <label class="error-msg text-red">@error('ppmp_type'){{$message}}@enderror
                </label>
              </div>

              @if($ppmp_type==="regular")
              ...
              <div class="form-group col-xs-12 col-sm-12 col-lg-11 mb-0">
                <label for="sppmp_number">SPPMP Number<span class="text-red"></span></label>
                <input type="text" id="sppmp_number" name="sppmp_number" class="form-control form-control-sm"  value="{{old('sppmp_number')}}">
                <label class="error-msg text-red">@error('ppmp_year'){{$message}}@enderror
                </label>
              </div>
              @endif
    ...
    <div class="card shadow border-0">
      ...
      <div class="card-body">
        <div class="col-sm-12" id="filter">
          <form class="row" id="ppmp_filter" method="post" action="{{route('archive.filter_archive_ppmp')}}">
            @csrf

            <!-- PPMP TYPE -->
            <div class="form-group col-xs-12 col-sm-6 col-lg-6 mb-0 d-none">
              <label for="ppmp_type">PPMP Type <span class="text-red">*</span></label>
              <input type="text" id="ppmp_type" name="ppmp_type" class="form-control" value="{{$ppmp_type}}" >
              <label class="error-msg text-red" >@error('ppmp_type'){{$message}}@enderror
              </label>
            </div>

            <div class="form-group col-xs-12 col-sm-2 col-lg-2 mb-0">
              <label for="ppmp_year">PPMP  Year<span class="text-red">*</span></label>
              <input type="text" id="ppmp_year" name="ppmp_year" class=" yearpicker form-control  form-control-sm" >
              <label class="error-msg text-red" >@error('ppmp_year'){{$message}}@enderror
              </label>
            </div>
            @if($ppmp_type==="regular")
            ...
            @else
            <div class="form-group col-xs-12 col-sm-2 col-lg-2 mb-0">
              <label for="ppmp_group">SPPMP Number<span class="text-red">*</span></label>
              <input type="text" id="ppmp_group" name="ppmp_group" class="form-control  form-control-sm" value="{{old('ppmp_group')}}" >
              <label class="error-msg text-red" >@error('ppmp_group'){{$message}}@enderror
              </label>
            </div>
            @endif
          </form>
          ...
          <table class="table table-bordered wrap" id="archive_table">
            <thead class="">
              <tr class="bg-primary text-white" >
                ...
                <th class="text-center">PPMP/SPPMP No.</th>
...
// table data
let data= @json(session('archive_ppmps'));

if(data==null){
  data= @json($archive_ppmps);
}
...
// inputs
if("{{old('ppmp_year')}}"==""){
  $("#ppmp_year").datepicker("update","{{$year}}");
}
else{
  $("#ppmp_year").datepicker("update","{{old('ppmp_year')}}");
}
...
$("#ppmp_year").change(function () {
  $("#ppmp_filter").submit();
});

$("#ppmp_group").change(function () {
  $("#ppmp_filter").submit();
});
...
$('#archive_table tbody').on('click', '.edit-btn', function(e) {
  ...
  $("#archive_title").html("Update Attachment/s");
  $('#id').val(row.id);
  $("#year").datepicker("update",row.ppmp_year+'');
  $('#edit-id').val(row.id);
  $("#source_of_fund").val(row.fund_category_id);
  $("#sppmp_number").val(row.ppmp_no);
  ...
  $.ajax({
    'url': "{{route('archive.get_ppmp_attachments')}}",
    'data': {
      "_token": "{{ csrf_token() }}",
      "id" : row.id,
    },
    ...
    'success': function(data) {
      if(data.length>0){
        data.forEach((item, i) => {
          ...
          existing_attachment=existing_attachment+'<a href="/archive/view_ppmp_attachment/'+item.id+'" target="_blank"> Attachment '+(i+1)+'</a>';
        ...
        $(".remove_existing_attachment").click(function() {
          ...
          Swal.fire({
            ...
            if (result.value == true) {
              $.ajax({
                'url': "{{route('archive.delete_ppmp_attachment')}}",
                'data': {
                  "_token": "{{ csrf_token() }}",
...
$('#archive_table tbody').on('click', '.delete-btn', function (e) {
  ...
  Swal.fire({
    ...
    if (result.value == true) {
      $.ajax({
        'url': "{{route('archive.delete_ppmp_attachments')}}",
        'data': {
          "_token": "{{ csrf_token() }}",
          "id" :row.id,
        },
