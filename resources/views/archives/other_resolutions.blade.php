                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h3 class="modal-title" id="archive_title">Other Resolution</h3>
                            ...
                            <form class="col-sm-12" method="POST" id="archive_form" action="{{route('archive.submit_resolution')}}" enctype="multipart/form-data">
                                ...
                                <div class="row d-flex">
                                    ...
                                    <div class="form-group col-xs-6 col-sm-6 col-lg-6 mb-0 d-none">
                                        ...
                                        <input type="text" id="type" name="type" class="form-control form-control-sm datepicker" value="OTHERS">
                                    ...
                                    <div class="form-group col-xs-12 col-sm-12 col-lg-12 mb-0" id="remarks_container">
...
<script>
    ...
    var table = $('#archive_table').DataTable({
        ...
        , columns: [{
                ...
                , render: function(data, type, row) {
                    ...
                    if (data == 1) {
                        return '@if(in_array("update",$user_privilege))<button class="btn btn-sm btn-success edit-btn  " resolution_id="' + row.resolution_id + '" data-toggle="tooltip" data-placement="top" title="Edit"><i class="ni ni-ruler-pencil"></i></button>@endif @if(in_array("delete",$user_privilege)) <button data-toggle="tooltip" data-placement="top" title="Delete" class="btn btn-sm btn btn-sm btn-danger delete-btn"><i class="ni ni-basket text-white"></i></button>@endif <a class="btn btn-sm shadow-0 border-0 btn-primary text-white" target="_blank" data-toggle="tooltip" data-placement="top" title="View Attachment/s" href="view_resolution_attachments/' + row.resolution_id + '"><i class="ni ni-tv-2"></i></a>';

                    } else {
                        return '@if(in_array("delete",$user_privilege)) <button data-toggle="tooltip" data-placement="top" title="Delete" class="btn btn-sm btn btn-sm btn-danger delete-btn"><i class="ni ni-basket text-white"></i></button>@endif';
    ...
    let resolution_number = @json(old('resolution_number'));
    let resolution_date = @json(old('resolution_date'));
    ...
    if ("{{session('message')}}") {
        if ("{{session('message')}}" == "missing_attachment") {
            ...
            swal.fire({
                title: `Duplicate Resolution Number`
                , text: 'Please validate resolution number.'
                , buttonsStyling: false
                , customClass: {
                    confirmButton: 'btn btn-sm btn-warning'
                , }
    ...
    $('#archive_table tbody').on('click', '.edit-btn', function(e) {
        ...
        $('#resolution_number').val(row.resolution_number);
        $('#resolution_date').val(row.resolution_date);
        $('#remarks').val(row.resolution_remarks);
        $('#id').val(row.resolution_id);
