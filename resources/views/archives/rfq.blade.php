        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              action="{{route('archive.submit_rfq')}}" enctype="multipart/form-data">
              @csrf
      ...
      <div class="card shadow border-0">
        ...
        <div class="card-body">
          <div class="col-sm-12" id="filter">
            <form class="row" id="app_filter" method="post" action="{{route('archive.filter_rfq')}}">
              ...
              <div class="form-group col-xs-12 col-sm-2 col-lg-2 mb-0">
                ...
                <select type="text" id="status" name="status" class="form-control form-control-sm" >
                  <option {{ old('status') === "with_or_without_itbrfq_attachment" ? "selected" : "" }} value="with_or_without_itbrfq_attachment">All</option>
                  <option {{ old('status') === "with_itbrfq_attachment" ? "selected" : "" }} value="with_itbrfq_attachment">With Attachment</option>
                  <option {{ old('status') === "without_itbrfq_attachment" ? "selected" : "" }} value="without_itbrfq_attachment">Without Attachment</option>
                </select>
              ...
              <div class="form-group col-xs-12 col-sm-2 col-lg-2 mb-0">
                ...
                <select type="" id="mode_of_procurement" name="mode_of_procurement" class="form-control form-control-sm" value="{{old('mode_of_procurement')}}" >
                  <option {{ old('mode_of_procurement') === "2" ? "selected" : "" }} value="2" >Small Value Procurement</option>
                  <option {{ old('mode_of_procurement') === "3" ? "selected" : "" }} value="3">Negotiated Procurement</option>
...
@if(in_array('update',$user_privilege))
  $('#archive_table tbody').on('click', '.edit-btn', function(e) {
    ...
    $.ajax({
      'url': "{{route('archive.get_rfq_attachments')}}",
      'data': {
        "_token": "{{ csrf_token() }}",
      ...
      'success': function(data) {
        if(data.length>0){
          data.forEach((item, i) => {
            ...
            existing_attachment=existing_attachment+'<a href="/archive/view_rfq_attachment/'+item.id+'" target="_blank"> Attachment '+(i+1)+'</a>';
          ...
          $(".remove_existing_attachment").click(function() {
            ...
            Swal.fire({
              ...
              if (result.value == true) {
                $.ajax({
                  'url': "{{route('archive.delete_rfq_attachment')}}",
                  'data': {
                    "_token": "{{ csrf_token() }}",
...
@if(in_array('delete',$user_privilege))
  $('#archive_table tbody').on('click', '.delete-btn', function (e) {
    ...
    Swal.fire({
      ...
      if (result.value == true) {
        $.ajax({
          'url': "{{route('archive.delete_rfq_attachments')}}",
          'data': {
            "_token": "{{ csrf_token() }}",
            "id" :row.procact_id,
          },
