        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              action="{{route('archive.submit_transmittal')}}" enctype="multipart/form-data">
              @csrf
              <div class="row d-flex">

                <div class="form-group col-xs-12 col-sm-12 col-lg-12 d-none">
                  <label for="trasmittal_id">Transmittal ID <span class="text-red">*</span></label>
                  <input type="text" trasmittal_id="trasmittal_id" name="trasmittal_id" class="form-control form-control-sm" readonly value="{{old('trasmittal_id')}}">
                  <label class="error-msg text-red">@error('trasmittal_id'){{$message}}@enderror
                  </label>
                </div>
                ...
                <div class="form-group col-xs-12 col-sm-12 col-lg-12 d-none">
                  ...
                  <input type="text" id="plan_id" name="plan_id" class="form-control form-control-sm" readonly value="{{old('plan_id')}}">
                  <label class="error-msg text-red">@error('plan_id'){{$message}}@enderror
                  </label>
                </div>

                <div class="form-group col-xs-12 col-sm-12 col-lg-12 d-none">
                  <label for="project_bid">Project Bid<span class="text-red">*</span></label>
                ...
                <div class="form-group col-xs-12 col-sm-12 col-lg-12 mb-0">
                  <label for="date_received_by_coa">Date Received By COA<span class="text-red">*</span></label>
                  <input type="text" id="date_received_by_coa" name="date_received_by_coa" class="form-control form-control-sm bg-white datepicker"   value="{{old('date_received_by_coa')}}" >
                  <label class="error-msg text-red" >@error('date_received_by_coa'){{$message}}@enderror
                  </label>
                </div>
      ...
      <div class="card shadow border-0">
        ...
        <div class="card-body">
          <div class="col-sm-12" id="filter">
            <form class="row" id="filter_notice_to_proceed" method="post" action="{{route('archive.filter_transmittals')}}">
              @csrf
              <!-- year -->
            ...
            <table class="table table-bordered" id="archive_table">
              <thead class="">
                <tr class="bg-primary text-white" >
                  ...
                  <th class="text-center">Date Received By COA</th>
...
var table=  $('#archive_table').DataTable({
  ...
  data: data,
  columns: [
    {"data": "transmittal_id",render: function( data, type, row ) {
      if(data!=null){
        return '<div style="white-space: nowrap">@if(in_array("update",$user_privilege))<button class="btn btn-sm btn-success edit-btn" id="edit-btn-'+row.transmittal_id+'" transmittal_id="'+row.transmittal_id+'" data-toggle="tooltip" data-placement="top" title="Edit" ><i class="ni ni-ruler-pencil"></i></button>@endif <a  class="btn btn-sm shadow-0 border-0 btn-primary text-white pb-1" target="_blank" data-toggle="tooltip" data-placement="top" title="View Attachment/s"  href="/archive/view_transmittal_attachments/' + row.transmittal_id + '"><i class="ni ni-tv-2"></i></a></div>';

      }
      else{
        return "@if(in_array('add',$user_privilege))<button type='button' type='button' class='btn btn-sm btn-primary add-btn' transmittal_id='"+row.transmittal_id+"' data-toggle='tooltip' data-placement='top' title='Add Attachment'><i class='ni ni-fat-add'></i></button>@endif";
      }
    }},
    { "data": "transmittal_id" },
    { "data": "plan_id" },
    ...
    { "data": "date_received_by_coa" },
    { "data": "transmittal_remarks" },
    { "data": "name" }
  ],
  language: {
    paginate: {
...
$('#archive_table tbody').on('click', '.add-btn', function(e) {
  ...
  $('#project_bid').val(row.project_bid_id);
...
$('#archive_table tbody').on('click', '.edit-btn', function(e) {
  ...
  let id=$(this).attr('transmittal_id');
  var row = table.row($(this).parents('tr')).data();
  $("#archive_form")[0].reset();
  ...
  if(row.transmittal_date_received_by_coa!=null){
    $("#date_received_by_coa").val(moment(row.transmittal_date_received_by_coa,'Y-MM-DD').format("MM/DD/YYYY"));
  }
  else{
    $("#date_received_by_coa").val('');
  }

  if(row.transmittal_date_released!=null){
    $("#date_released").datepicker('setDate',moment(row.transmittal_date_released,'Y-MM-DD').format("MM/DD/YYYY"));
  }
  else{
    $("#date_released").val('');
  }

  if(row.transmittal_date_received_by_contractor!=null){
    $("#date_received_by_contractor").datepicker('setDate',moment(row.transmittal_date_received_by_contractor,'Y-MM-DD').format("MM/DD/YYYY"));
  }
  else{
  ...
  if(row.transmittal_date_received!=null){
    $("#date_received_by_bac").datepicker('setDate',moment(row.transmittal_date_received,'Y-MM-DD').format("MM/DD/YYYY"));
  }
  else{
  ...
  $("#remarks").val(row.transmittal_remarks);
  ...
  $.ajax({
    'url': "{{route('archive.get_archive_transmittal_attachments')}}",
    'data': {
      "_token": "{{ csrf_token() }}",
    ...
    'success': function(data) {
      if(data.length>0){
        data.forEach((item, i) => {
          ...
          existing_attachment=existing_attachment+'<a href="/archive/view_transmittal_attachment/'+item.id+'" target="_blank"> Attachment '+(i+1)+'</a>';
        ...
        $(".remove_existing_attachment").click(function() {
          ...
          Swal.fire({
            ...
            if (result.value == true) {
              $.ajax({
                'url': "{{route('archive.delete_transmittal_attachment')}}",
                'data': {
                  "_token": "{{ csrf_token() }}",
