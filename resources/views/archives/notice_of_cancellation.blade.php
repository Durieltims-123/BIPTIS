        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              action="{{route('archive.submit_minute')}}" enctype="multipart/form-data">
              @csrf
              <div class="row d-flex">

                <!--ID -->
                <div class="form-group col-xs-12 col-sm-12 col-lg-12 d-none mb-0">
                  <label for="id">ID <span class="text-red">*</span></label>
                  <input type="text" id="id" name="id" class="form-control form-control-sm" readonly
                  value="{{old('id')}}">
                  <label class="error-msg text-red" for="id">@error('id'){{$message}}@enderror</label>
                </div>

      
                 <!--Procact IDS -->
                 <div class="form-group col-xs-12 col-sm-12 col-lg-12 mb-0 d-none">
                     <label for="procacts">Project Activity Ids<span class="text-red">*</span></label>
                     <input type="text" id="procacts" name="procacts" class="form-control form-control-sm" readonly value="">
                     <label class="error-msg text-red" for="procacts">@error('procacts'){{$message}}@enderror</label>
                 </div>

                <!-- Notice Date -->
                <div class="form-group col-xs-6 col-sm-6 col-lg-6 mb-0">
                  <label for="notice_date">Notice Date<span class="text-red">*</span></label>
                  <input type="text" id="notice_date" name="notice_date"
                  class="form-control form-control-sm datepicker2" value="{{old('notice_date')}}">
                  <label class="error-msg text-red" for="notice_date">@error('notice_date'){{$message}}@enderror</label>
                </div>
                ...
                <div class="form-group col-xs-12 col-sm-12 col-lg-12 mb-0">
                    <label for="project_title">Select Project<span class="text-red"></span></label>
                 ...
                 <div class="col-xs-12 col-sm-12 col-lg-12 mt-1">
                     <label for="">Selected Projects<span class="text-red">*</span></label>
                      <div id="projects_div" class="m-1">
                ...
                <div class="form-group col-xs-12 col-sm-12 col-lg-12">
                  <p for="">Attachment/s <span class="text-red">*</span></p>
      ...
      <div class="card shadow border-0">
        ...
        <div class="card-body">
          <div class="col-sm-12" id="filter">
            <form class="row" id="filter_archive_minute" method="post" action="{{route('archive.filter_minute')}}">
              @csrf
              <!-- project year -->
...
let old_year=@json(old('year'));
  ...
  , source: function(request, response) {
      ...
      , change: function(event, ui) {
        if(ui.item!=null){
          let values=$("#procacts").val().split(',');
          if($("#procacts").val()===""){
            $("#procacts").val(ui.item.id);
            $("#projects_div .row").last().after('<div class="row project_group" id="'+ui.item.id+'"><div class="col-md-11"><label type="text" class="form-control label">'+ui.item.value+'</label></div><div class="col-md-1 mt-2"><button type="button" class="btn btn-sm btn-danger p-1 remove_attachment"><i class="ni ni-fat-remove"></i></button></div></div>');

          }
          else{
            if(values.includes(ui.item.id+"")===false){
               $("#procacts").val($("#procacts").val()+','+ui.item.id);
               $("#projects_div .row").last().after('<div class="row project_group" id="'+ui.item.id+'"><div class="col-md-11"><label type="text" class="form-control label">'+ui.item.value+'</label></div><div class="col-md-1 mt-2"><button type="button" class="btn btn-sm btn-danger p-1 remove_attachment"><i class="ni ni-fat-remove"></i></button></div></div>');
          ...
          $(".remove_attachment").click(function() {
               let id= $(this).parents('.project_group').attr("id");
               let values=$("#procacts").val().split(',');
               const result = values.filter(value => value!=id);
               $("#procacts").val(result.join(",").toString());
               $(this).parents('.project_group').remove();
          });
...
$('#archive_table tbody').on('click', '.edit-btn', function (e) {
  ...
  table.rows().deselect();
  $("#id").val(row.id);
  $("#date").datepicker('setDate', moment(row.date).format('MM/DD/YYYY'));
  ...
  $.ajax({
    'url': "{{route('archive.get_archive_minute_attachments')}}",
    'data': {
      "_token": "{{ csrf_token() }}",
      "archive_minute_id" : row.id,
    },
    'method': "post",
    'success': function(data) {
      if(data.length>0){
        data.forEach((item, i) => {
          ...
          existing_attachment=existing_attachment+'<a href="view_minute_attachment/'+item.id+'" target="_blank"> Attachment '+(i+1)+'</a>';
        ...
        $(".remove_existing_attachment").click(function() {
          ...
          Swal.fire({
            ...
            if (result.value == true) {
              $.ajax({
                'url': "{{route('archive.delete_minute_attachment')}}",
                'data': {
                  "_token": "{{ csrf_token() }}",
...
$('#archive_table tbody').on('click', '.delete-btn', function (e) {
  ...
  Swal.fire({
    ...
    if (result.value == true) {
      $.ajax({
        'url': "{{route('archive.delete_minute')}}",
        'data': {
          "_token": "{{ csrf_token() }}",
