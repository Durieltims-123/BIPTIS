            ...
            <div class="modal-body">
              ...
              <div class="row d-flex">
                ...
                <div class="form-group col-xs-12 col-sm-12 col-lg-12 d-none">
                  <label for="transmittal_id">Transmittal ID <span class="text-red">*</span></label>
                  <input type="text" id="transmittal_id" name="transmittal_id" class="form-control form-control-sm" readonly value="{{old('transmittal_id')}}">
                  <label class="error-msg text-red">@error('transmittal_id'){{$message}}@enderror
                  </label>
                </div>
                ...
                <div class="form-group col-xs-12 col-sm-12 col-lg-12 mb-0">
                  ...
                  <input type="text" id="project_title" name="project_title" class="form-control form-control-sm bg-white"   value="{{old('project_title')}}" >
                ...
                <div class="form-group col-xs-12 col-sm-12 col-lg-12 d-none">
                  ...
                  <input type="text" id="project_number" name="project_number" class="form-control form-control-sm" readonly value="{{old('project_number')}}">
      ...
      <div class="card shadow border-0">
        ...
        <div class="card-body">
          <div class="col-sm-12" id="filter">
            <form class="row" id="filter_archive_transmittal" method="post" action="{{route('archive.filter_transmittals')}}">
              @csrf
              <!-- project year -->
...
var project_title_autocomplete_init = {
  ...
  source: function(request, response){
    ...
    $.ajax({
      'url': '{{route("archive.autocomplete_project_with_ntps")}}',
      'data': {
        "_token": "{{ csrf_token() }}",
        "term" : request.term
      },
...
$("#filter_btn").click(function () {
  $("#filter_archive_transmittal").submit();
});
...
$('#archive_table tbody').on('click', '.edit-btn', function (e) {
  ...
  table.rows().deselect();
  $("#transmittal_id").val(row.transmittal_id);
  ...
  $("#date_received_by_coa").datepicker('setDate', moment(row.date_received_by_coa).format('MM/DD/YYYY'));
  ...
  $.ajax({
    ...
    'data': {
      "_token": "{{ csrf_token() }}",
      "archive_transmittal_id" : row.transmittal_id,
    },
    'method': "post",
    'success': function(data) {
      if(data.length>0){
        data.forEach((item, i) => {
          ...
          existing_attachment=existing_attachment+'<a href="view_transmittal_attachment/'+item.id+'" target="_blank"> Attachment '+(i+1)+'</a>';
...
$('#archive_table tbody').on('click', '.delete-btn', function (e) {
  ...
  Swal.fire({
    ...
    if (result.value == true) {
      $.ajax({
        'url': "{{route('archive.delete_transmittal')}}",
        'data': {
          "_token": "{{ csrf_token() }}",
          "id" : $(this).attr('minute_id'),
        },
        'method': "post",
        'success': function(data) {
          if(data=="success"){
