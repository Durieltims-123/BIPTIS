                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form class="col-sm-12" method="POST" id="archive_form" action="{{route('archive.submit_contract')}}" enctype="multipart/form-data">
                                @csrf
            ...
            <div class="card shadow border-0">
                ...
                <div class="card-body">
                    <div class="col-sm-12" id="filter">
                        <form class="row" id="filter_contract" method="post" action="{{route('archive.filter_contracts')}}">
                            @csrf
                            <!-- year -->
...
<script>
    ...
    var table = $('#archive_table').DataTable({
        ...
        , columns: [{
                ...
                , render: function(data, type, row) {
                    if (data == 1) {
                        return '<div style="white-space: nowrap"> @if(in_array("update",$user_privilege)) <button class="btn btn-sm btn-success edit-btn  " id="edit-btn-' + row.contract_id + '" contract_id="' + row.contract_id + '" data-toggle="tooltip" data-placement="top" title="Edit" ><i class="ni ni-ruler-pencil"></i></button>@endif <a  class="btn btn-sm shadow-0 border-0 btn-primary text-white" target="_blank" data-toggle="tooltip" data-placement="top" title="View Attachment/s"  href="/archive/view_contract_attachments/' + row.contract_id + '"><i class="ni ni-tv-2"></i></a></div>';

                    } else {
                        return "@if(in_array('add',$user_privilege))<button type='button' type='button' class='btn btn-sm btn-primary add-btn' contract_id='" + row.contract_id + "' data-toggle='tooltip' data-placement='top' title='Add Attachment'><i class='ni ni-fat-add'></i></button>@endif";
                    }
                }
            }
            , {
    ...
    if ("{{session('message')}}") {
        if ("{{session('message')}}" == "missing_attachment") {
            ...
            swal.fire({
                ...
                , text: 'Please Verify Performance bond Details'
                , buttonsStyling: false
                , customClass: {
                    confirmButton: 'btn btn-sm btn-warning'
                , }
    ...
    $('#archive_table tbody').on('click', '.add-btn', function(e) {
        ...
        let id = $(this).attr('contract_id');
        var row = table.row($(this).parents('tr')).data();
        $("#archive_form")[0].reset();
        $("#archive_title").html("Add Contract  Attachment/s");
        $('#id').val(id);
        $('#edit-id').val('');
        $("#existing_attachments").html('');

        if (row.contract_date_generated != null) {
            $("#date_generated").val(moment(row.contract_date_generated, 'Y-MM-DD').format("MM/DD/YYYY"));
        } else {
            $("#date_generated").val('');
        }

        if (row.contract_release_date != null) {
            $("#date_released").val(moment(row.contract_release_date, 'Y-MM-DD').format("MM/DD/YYYY"));
        } else {
            $("#date_released").val('');
        }

        if (row.contract_date_received_contractor != null) {
            $("#date_received_by_contractor").val(moment(row.contract_date_received_contractor, 'Y-MM-DD').format("MM/DD/YYYY"));
        } else {
            $("#date_received_by_contractor").val('');
        }

        if (row.contract_date_of_notarization != null) {
            $("#date_of_notarization").val(moment(row.contract_date_of_notarization, 'Y-MM-DD').format("MM/DD/YYYY"));
        } else {
            $("#date_of_notarization").val('');
        }

        if (row.contract_receive_date != null) {
            $("#date_received_by_bac").val(moment(row.contract_receive_date, 'Y-MM-DD').format("MM/DD/YYYY"));
        } else {
            $("#date_received_by_bac").val('');
        }
        $("#remarks").val(row.contract_remarks);
    ...
    $('#archive_table tbody').on('click', '.edit-btn', function(e) {
        ...
        var row = table.row($(this).parents('tr')).data();
        $("#archive_form")[0].reset();
        $("#archive_title").html("Update Contract  Attachment/s");
        $('#id').val(id);
        $('#edit-id').val(id);
        if (row.contract_date_generated != null) {
            $("#date_generated").datepicker('update', moment(row.contract_date_generated, 'Y-MM-DD').format("MM/DD/YYYY"));
        } else {
            $("#date_generated").val('');
        }

        if (row.contract_release_date != null) {
            $("#date_released").datepicker('update', moment(row.contract_release_date, 'Y-MM-DD').format("MM/DD/YYYY"));
        } else {
            $("#date_released").val('');
        }

        if (row.contract_date_received_contractor != null) {
            $("#date_received_by_contractor").datepicker('update', moment(row.contract_date_received_contractor, 'Y-MM-DD').format("MM/DD/YYYY"));
        } else {
            $("#date_received_by_contractor").val('');
        }

        if (row.contract_date_of_notarization != null) {
            $("#date_of_notarization").datepicker('update', moment(row.contract_date_of_notarization, 'Y-MM-DD').format("MM/DD/YYYY"));
        } else {
            $("#date_of_notarization").val('');
        }

        if (row.contract_receive_date != null) {
            $("#date_received_by_bac").datepicker('update', moment(row.contract_receive_date, 'Y-MM-DD').format("MM/DD/YYYY"));
        } else {
            $("#date_received_by_bac").val('');
        }
        $("#remarks").val(row.contract_remarks);
        ...
        $.ajax({
            'url': "{{route('archive.get_archive_contract_attachments')}}"
            , 'data': {
                "_token": "{{ csrf_token() }}"
            ...
            , 'success': function(data) {
                if (data.length > 0) {
                    data.forEach((item, i) => {
                        ...
                        existing_attachment = existing_attachment + '<a href="/archive/view_contract_attachment/' + item.id + '" target="_blank"> Attachment ' + (i + 1) + '</a>';
                    ...
                    $(".remove_existing_attachment").click(function() {
                        ...
                        Swal.fire({
                            ...
                            if (result.value == true) {
                                $.ajax({
                                    'url': "{{route('archive.delete_contract_attachment')}}"
                                    , 'data': {
                                        "_token": "{{ csrf_token() }}"
