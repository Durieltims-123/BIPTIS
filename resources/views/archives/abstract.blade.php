        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              action="{{route('archive.submit_abstract')}}" enctype="multipart/form-data">
              @csrf
              <div class="row d-flex">
                ...
                <div class="form-group col-xs-6 col-sm-6 col-lg-6">
                  <label for="date_opened">Date Opened <span class="text-red">*</span></label>
      ...
      <div class="card shadow border-0">
        ...
        <div class="card-body">
          <div class="col-sm-12" id="filter">
            <form class="row" id="filter_archive_abstract" method="post" action="{{route('archive.filter_abstract')}}">
              @csrf
              <!-- project year -->
  ...
  columns: [
    {
      ...
      render: function (data, type, row) {
        var id = row.id;
        return ' @if(in_array("update",$user_privilege)) <button class="btn btn-sm btn-success edit-btn  " id="edit-btn-'+id+'" data-toggle="tooltip" data-placement="top" title="Edit"><i class="ni ni-ruler-pencil"></i></button> @endif  @if(in_array("delete",$user_privilege))<button class="btn btn-sm btn-danger delete-btn  " abstract_id="'+id+'" data-toggle="tooltip" data-placement="top" title="Delete"><i class="ni ni-basket text-white"></i></button>@endif <a  class="btn btn-sm shadow-0 border-0 btn-primary text-white" target="_blank"  href="/archive/view_abstract_attachments/' + id + '"><i class="ni ni-tv-2" data-toggle="tooltip" data-placement="top" title="View Attachment/s"></i></a>';
      }
    },
    { "data": "id" },
    { "data": "date_opened" ,
...
$("#filter_btn").click(function () {
  $("#filter_archive_abstract").submit();
});
...
$('#archive_table tbody').on('click', '.edit-btn', function (e) {
  ...
  $.ajax({
    'url': "{{route('archive.get_archive_abstract_attachments')}}",
    'data': {
      "_token": "{{ csrf_token() }}",
      "archive_abstract_id" : row.id,
    },
    'method': "post",
    'success': function(data) {
      if(data.length>0){
        data.forEach((item, i) => {
          ...
          existing_attachment=existing_attachment+'<a href="view_abstract_attachment/'+item.id+'" target="_blank"> Attachment '+(i+1)+'</a>';
        ...
        $(".remove_existing_attachment").click(function() {
          ...
          Swal.fire({
            ...
            if (result.value == true) {
              $.ajax({
                'url': "{{route('archive.delete_abstract_attachment')}}",
                'data': {
                  "_token": "{{ csrf_token() }}",
...
$('#archive_table tbody').on('click', '.delete-btn', function (e) {
  ...
  Swal.fire({
    ...
    if (result.value == true) {
      $.ajax({
        'url': "{{route('archive.delete_abstract')}}",
        'data': {
          "_token": "{{ csrf_token() }}",
          "id" : $(this).attr('abstract_id'),
        },
        'method': "post",
        'success': function(data) {
          if(data=="success"){
