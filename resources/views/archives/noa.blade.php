        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              action="{{route('archive.submit_notice_of_award')}}" enctype="multipart/form-data">
              @csrf
      ...
      <div class="card shadow border-0">
        ...
        <div class="card-body">
          <div class="col-sm-12" id="filter">
            <form class="row" id="filter_notice_of_award" method="post" action="{{route('archive.filter_notice_of_awards')}}">
              @csrf
              <!-- year -->
...
var table=  $('#archive_table').DataTable({
  ...
  columns: [
    {"data": "with_attachment",render: function( data, type, row ) {
      if(data==1){
        return '<div style="white-space: nowrap">@if(in_array("update",$user_privilege)) <button class="btn btn-sm btn-success edit-btn  " id="edit-btn-'+row.notice_award_id+'" notice_award_id="'+row.notice_award_id+'" data-toggle="tooltip" data-placement="top" title="Edit" ><i class="ni ni-ruler-pencil"></i></button>@endif<a  class="btn btn-sm shadow-0 border-0 btn-primary text-white" target="_blank" data-toggle="tooltip" data-placement="top" title="View Attachment/s"  href="/archive/view_noa_attachments/' + row.notice_award_id + '"><i class="ni ni-tv-2"></i></a></div>';
      }
      else{
        return "@if(in_array('add',$user_privilege))<button type='button' type='button' class='btn btn-sm btn-primary add-btn' notice_award_id='"+row.notice_award_id+"' data-toggle='tooltip' data-placement='top' title='Add Attachment'><i class='ni ni-fat-add'></i></button>@endif";
      }
    }},
    { "data": "date_uploaded" },
...
$('#archive_table tbody').on('click', '.add-btn', function(e) {
  ...
  let id=$(this).attr('notice_award_id');
  var row = table.row($(this).parents('tr')).data();
  $("#archive_form")[0].reset();
  $("#archive_title").html("Add Notice of Award Attachment/s");
  $('#id').val(id);
  $('#edit-id').val('');
  ...
  if(row.date_released!=null){
    $("#date_released").val(moment(row.date_released,'Y-MM-DD').format("MM/DD/YYYY"));
  }
  else{
    $("#date_released").val('');
  }
  ...
  if(row.date_received_by_contractor!=null){
    $("#date_received_by_contractor").val(moment(row.date_received_by_contractor,'Y-MM-DD').format("MM/DD/YYYY"));
  }
  else{
  ...
  if(row.date_received!=null){
    $("#date_received_by_bac").val(moment(row.date_received,'Y-MM-DD').format("MM/DD/YYYY"));
  }
  else{
    $("#date_received_by_bac").val('');
  }
  $("#remarks").val(row.noa_remarks);
...
$('#archive_table tbody').on('click', '.edit-btn', function(e) {
  ...
  var row = table.row($(this).parents('tr')).data();
  $("#archive_form")[0].reset();
  $("#archive_title").html("Update Notice of Award Attachment/s");
  $('#id').val(id);
  $('#edit-id').val(id);
  if(row.date_generated!=null){
    $("#date_generated").datepicker('update',moment(row.date_generated,'Y-MM-DD').format("MM/DD/YYYY"));
  }
  else{
    $("#date_generated").val('');
  }
  ...
  if(row.date_released!=null){
    $("#date_released").datepicker('update',moment(row.date_released,'Y-MM-DD').format("MM/DD/YYYY"));
  }
  else{
    $("#date_released").val('');
  }
  ...
  if(row.date_received_by_contractor!=null){
    $("#date_received_by_contractor").datepicker('update',moment(row.date_received_by_contractor,'Y-MM-DD').format("MM/DD/YYYY"));
  }
  else{
    $("#date_received_by_contractor").val('');
  }
  ...
  if(row.date_received!=null){
    $("#date_received_by_bac").datepicker('update',moment(row.date_received,'Y-MM-DD').format("MM/DD/YYYY"));
  }
  else{
    $("#date_received_by_bac").val('');
  }
  ...
  $.ajax({
    'url': "{{route('archive.get_archive_noa_attachments')}}",
    'data': {
      "_token": "{{ csrf_token() }}",
      "notice_award_id" : row.notice_award_id,
    },
    'method': "post",
    'success': function(data) {
      if(data.length>0){
        data.forEach((item, i) => {
          ...
          existing_attachment=existing_attachment+'<a href="/archive/view_noa_attachment/'+item.id+'" target="_blank"> Attachment '+(i+1)+'</a>';
        ...
        $(".remove_existing_attachment").click(function() {
          ...
          Swal.fire({
            ...
            if (result.value == true) {
              $.ajax({
                'url': "{{route('archive.delete_noa_attachment')}}",
                'data': {
                  "_token": "{{ csrf_token() }}",
