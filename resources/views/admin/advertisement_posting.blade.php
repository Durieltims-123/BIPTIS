        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h3 class="modal-title" id="form_modal_title">Set Advertisement/Posting Date</h3>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div class="modal-body pt-0">
              <form class="col-sm-12" method="POST" id="submit_advertisement_posting" action="/submit_advertisement_posting">
                @csrf
                <div class="row">
                  <div class="form-group col-xs-6 col-sm-6 col-lg-6 mx-auto">
                    <h5 for="advertisement_posting_date">Plan Id/s</h5>
                  ...
                  <div class="form-group col-xs-16 col-sm-6 col-md-6 mb-2  mb-0">
                    <h5 for="advertisement_posting_date">Advertisement/Posting Date</h5>
                    <input type="text" id="advertisement_posting_date" name="advertisement_posting_date" class="form-control form-control-sm datepicker" value="{{old('advertisement_posting_date')}}" >
            ...
            <table class="table table-bordered" id="app_table">
              <thead class="">
                <tr class="bg-primary text-white" >
                  ...
                  <th class="text-center">Advertisement/Posting Date</th>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td style="white-space: nowrap"><button class="btn btn-sm btn-primary">Set Date</button>@if($project_plan->current_cluster) <span class="badge bg-yellow">Clustered</span>@endif</td>
                ...
                <tr>
                  ...
                  <td style="white-space: nowrap"><button class="btn btn-sm btn-primary set_date_btn" >Set Date</button>@if($project_plan->current_cluster) <span class="badge bg-yellow">Clustered</span>@endif</td>
                  <td>{{ $project_plan->plan_id }}</td>
                  <td>{{ date("M d,Y", strtotime($project_plan->advertisement_start))}} - {{ date("M d,Y", strtotime($project_plan->advertisement_end))}}</td>
                  <td>{{ $project_plan->current_cluster }}</td>
                  <td>{{ $project_plan->app_group_no }}</td>
...
if(oldInputs>=2){
  ...
  else if('{{old("extend_plan_id")}}'!=''){
    $("#extend_modal").modal('show');
    $("#process").val("advertisement_posting");
  }
  else{
...
if("{{session('message')}}"){
  ...
  if("{{session('message')}}"=="less_range"){

    swal.fire({
      title: 'Date Error',
      text: 'Date should be Greater than Given Schedule Range.. ',
      buttonsStyling: false,
      confirmButtonClass: 'btn btn-sm btn-danger',
      icon: 'warning'
    });
    ...
    $("#plan_id").val('');
    $("#advertisement_posting_date").val('');
