    ...
    var table = $('#app_table').DataTable({
        ...
        , columns: [{
                ...
                , render: function(data, type, row) {
                    if (data == null || data == "") {
                        return "@if(in_array('update',$user_privilege))<button class='btn btn-sm btn btn-sm btn-primary add-btn' data-toggle='tooltip' data-placement='top' title='Add'><i class='ni ni-fat-add'></i></button>@endif";
                    } else {
                        if (row.additional_docs_status === "incomplete" || row.additional_docs_status === "complete" ) {

                            return "@if(in_array('update',$user_privilege))<div style='white-space: nowrap'><button class='btn btn-sm btn btn-sm btn-success edit-btn' data-toggle='tooltip' data-placement='top' title='Edit'><i class='ni ni-ruler-pencil'></i></button> @endif <a class='btn btn-sm shadow-0 border-0 btn-primary text-white' data-toggle='tooltip' data-placement='top' title='Download'  href='/generate_additional_docs/" + data + "'><i class='ni ni-cloud-download-95'></i></a></div>";
                        } else {
