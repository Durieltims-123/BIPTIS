          ...
          <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h3 class="modal-title" id="additional_docs_modal_title">Release Notice To Submit Documents</h3>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
              <div class="modal-body">
                <form class="col-sm-12" method="POST" id="release_documents_form" action="{{route('submit_release_notice_to_submit_documents')}}">
                  @csrf
        ...
        <div class="card shadow border-0">
          ...
          <div class="card-body">
            <div class="col-sm-12" id="filter">
              <form class="row" id="filter_project_bidder_additional_docs" method="post" action="{{route('filter_release_project_bidder_additional_docs')}}">
                @csrf
                <!-- project year -->
...
@push('custom-scripts')
  ...
  if("{{session('message')}}"){
    ...
    else if("{{session('message')}}"=="received_error"){
      Swal.fire({
        title: `Update Error`,
        text: 'Sorry! You cannot update received Notice To Submit Documents',
    ...
    columns: [
      ...
      render: function ( data, type, row ) {
        if(row.date_released==null){
          return "<div style='white-space: nowrap'> @if(in_array('release',$user_privilege))<button class='btn btn-sm btn btn-sm btn-warning release-btn' data-toggle='tooltip' data-placement='top' title='Release'><i class='ni ni-send'></i></button> @endif <a class='btn btn-sm btn btn-sm shadow-0 border-0 btn-primary text-white' data-toggle='tooltip' data-placement='top' title='Download'  href='/generate_additional_docs/"+data+"'><i class='ni ni-cloud-download-95'></i></a></div>";
        }
        else{
          return "<div style='white-space: nowrap'>@if(in_array('update',$user_privilege))<button class='btn btn-sm btn btn-sm btn-success release-btn' data-toggle='tooltip' data-placement='top' title='Edit'><i class='ni ni-ruler-pencil'></i></button> @endif <a class='btn btn-sm btn btn-sm shadow-0 border-0 btn-primary text-white' data-toggle='tooltip' data-placement='top' title='Download'  href='/generate_additional_docs/"+data+"'><i class='ni ni-cloud-download-95'></i></a></div>";
        }
      }
    },
    { "data": "pbard_id" },
  ...
  $("#svp_checklist_table").DataTable({
    ...
    $("#bidding_checklist_table").DataTable({
      ...
      $('#app_table tbody').on('click', '.add-btn', function (e) {
        ...
        $("#mode").val(data.mode);
        $("#additional_docs_modal_title").html("Release Documents Checklist");
      ...
      let errors=@json($errors->first('date_released'));
      if(errors.length!=""){
        if(@json(old('mode'))=="Bidding"){
        ...
        checkPresentMissingNADocs(@json(old('mode')));
        $("#additional_docs_modal").modal('show');
      }
      @if(in_array('update',$user_privilege)||in_array('release',$user_privilege))
      $('#app_table tbody').on('click', '.release-btn', function (e) {
        table.rows().deselect();
        var data=table.row($(this).parents('tr')).data();
        $("#release_documents_form")[0].reset();
        $("#pbard_id").val(data.pbard_id);
        ...
        $("#mode").val(data.mode);
        $("#additional_docs_modal_title").html("Release Additional Documents");
        if(data.mode=="Bidding"){
