                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h3 class="modal-title" id="release_modal_title">@if(old('bid_doc_id')!=="")Update @else Add @endif Release Bid Doc </h3>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <form class="col-sm-12" method="POST" id="release_form" action="submit_release_bid_doc">
                                @csrf
                                <div class="row d-flex">

                                    <div class="form-group col-xs-6 col-sm-6 col-lg-6 d-none">
                                        <label for="bid_doc_id">Bid Doc ID
                                        </label>
                                        <input type="text" id="bid_doc_id" name="bid_doc_id" class="form-control form-control-sm" value="{{old('bid_doc_id')}}">
                                        <label class="error-msg text-red">@error('bid_doc_id'){{$message}}@enderror
                                        </label>
                                    </div>
                                    ...
                                    <!-- date released -->
                                    <div class="form-group col-xs-6 col-sm-6 col-lg-6 ">
                                        <label for="date_released">Date Released<span class="text-red">*</span>
                                        </label>
                                    ...
                                    <!-- OR Number -->
                                    <div class="form-group col-xs-6 col-sm-6 col-lg-6 ">
                                        <label for="control_number">OR Number<span class="text-red">*</span>
                                        </label>
                                        <input type="text" id="control_number" name="control_number" class="form-control form-control-sm" value="{{old('control_number')}}">
                                        <label class="error-msg text-red">@error('control_number')The OR Number is required! @enderror
                                        </label>
                                    </div>
            ...
            <div class="card shadow border-0">
                ...
                <div class="card-body">
                    <div class="col-sm-12" id="filter">
                        <form class="row" id="filter_bid_doc" method="post" action="{{route('filter_bid_doc')}}">
                            @csrf
                            <!--  year -->
                            <div class="form-group col-xs-4 col-sm-3 col-lg-3 mb-0">
                                <label for="year" class="input-sm">Year Bidding Documents Released
                                </label>
...
@push('custom-scripts')
<script>
    let data = @json($released_bid_docs);
    if (@json(old('year')) != null) {
        data = @json(session('released_bid_docs'));
    }
    // datatables
    ...
    if ("{{session('message')}}") {
        if ("{{session('message')}}" == "duplicate") {
            ...
            Swal.fire({
                ...
                , text: 'Successfully deleted bid_doc'
            ...
            swal.fire({
                title: `Error`
                , text: 'Sorry, You cannot manipulate bid_doc after Submission/Opening of bids!'
                , buttonsStyling: false
            ...
            swal.fire({
                title: `Error`
                , text: 'You cannot delete Received Bid Docs'
                , buttonsStyling: false
                , confirmButtonClass: 'btn btn-sm btn-danger'
                , icon: 'warning'
            });
    ...
    $(".add_release").click(function functionName() {
        ...
        $("#release_modal_title").html("Release Bid Docs ");
    ...
    $('#app_table tbody').on('click', '.delete_btn', function(e) {
        var row = table.row($(this).parents('tr')).data();
        Swal.fire({
            text: 'Do you want to delete Bid Doc?'
            ...
            if (result.value == true) {
                window.location.href = "/delete_bid_doc/" + row.bid_doc_id;
            }
        });
