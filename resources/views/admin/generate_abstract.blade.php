    ...
    <div class="card shadow">
      <div class="card shadow border-0">
        <div class="card-header">
          <h2 id="title">Generate Abstract</h2>
        </div>
        <div class="card-body">
          <form class="col-sm-8 mx-auto" method="POST" id="release_form" action="/submit_generate_abstract">
            @csrf
            <div class="row d-flex">
              ...
              <div class="form-group col-xs-6 col-sm-6 col-lg-6 mt-4">
                ...
                <a class="btn btn-info text-center" target="_blank" href="/download_abstract/{{date('Y-m-d',strtotime(old('date_opened')))}}">Download</a>
                @endif
              </div>
            ...
            <table class="table table-bordered " id="app_table">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td rowspan="">{{$project_plan->project_no}}</td>
                  <td rowspan=""><div class='text-wrap' style="width:200px">{{$project_plan->project_title}}</div></td>
                  ...
                  <td>{{$bidder->proposed_bid}}</td>
                  <td>{{$bidder->bid_as_evaluated}}</td>
                  <td>{{$bidder->rank}}</td>
                </tr>
                @endforeach
                ...
                <tr>
                  ...
                  <td rowspan="">{{$project_plan->project_cost}}</td>
                  <td rowspan="">{{$project_plan->source_of_fund}}</td>
                  <td colspan="">@if($project_plan->mode_id===1)No Bidders @else No Quotations @endif</td>
                  <td></td>
                  <td></td>
                  <td></td>
                  <td></td>
