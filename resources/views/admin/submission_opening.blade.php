        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            ...
            <div class="modal-body pt-0">
              <form class="col-sm-12" method="POST" id="submit_submission_opening_of_bid" action="/submit_submission_opening_of_bid">
                @csrf
                <div class="row">
                  <div class="form-group col-xs-6 col-sm-6 col-lg-6 mx-auto">
                    <h5 for="submission_opening_of_bid_date">Plan Id/s</h5>
                  ...
                  <div class="form-group col-xs-16 col-sm-6 col-md-6 mb-2  mb-0">
                    <h5 for="submission_opening_of_bid_date">Submission/Opening Date</h5>
                    <input type="text" id="submission_opening_of_bid_date" name="submission_opening_of_bid_date" class="form-control form-control-sm datepicker" value="{{old('submission_opening_of_bid_date')}}" >
                    <label class="error-msg text-red" >@error('submission_opening_of_bid_date'){{$message}}@enderror</label>
                  </div>
                  <div class="form-group col-xs-16 col-sm-6 col-md-6 mb-2  mb-0">
                    <h5 for="submission_opening_of_bid_time">Submission/Opening Time</h5>
                    <input type="text" id="submission_opening_of_bid_time" name="submission_opening_of_bid_time" class="form-control form-control-sm timepicker bg-white" value="{{old('submission_opening_of_bid_time')}}" readonly>
            ...
            <table class="table table-bordered" id="app_table">
              <thead class="">
                <tr class="bg-primary text-white" >
                  ...
                  <th class="text-center">Submission/Opening Date</th>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td style="white-space: nowrap">
                    ...
                    <a class="btn btn-sm btn btn-sm btn-success" target='_blank' href="/project_bidders/{{$project_plan->plan_id}}" >Bidders:{{$project_plan->bidder_count}}</a>
                    @else
                    <a class="btn btn-sm btn btn-sm bg-teal text-white" target='_blank' href="/project_bidders/{{$project_plan->plan_id}}" >Bidders:{{$project_plan->bidder_count}}</a>
                    @endif
                    @if($project_plan->re_bid_count<=3||$project_plan->re_bid_count==null)
                    @if($project_plan->re_bid_count=='SVP')
                    <button class="btn btn-sm btn btn-sm btn-warning rebid_btn" >Another SVP</button>
                    @else
                    ...
                    @endif
                    @if($project_plan->current_cluster) <span class="badge bg-yellow">Clustered</span>@endif
                  </td>
...
if(oldInputs>2){
  ...
  if('{{old("extend_plan_id")}}'!=''){
    ...
    $("#process").val("submission_opening");
  }
  if('{{old("plan_id")}}'!=''){
...
if("{{session('message')}}"){
  if("{{session('message')}}"=="range_error"){
    ...
    $("#plan_id").val('');
    $("#submission_opening_of_bid_date").val('');
...
$("#extend_btn").click(function functionName() {
  if(table.rows( { selected: true } ).count()<1){
    ...
    let rows = table.rows( { selected: true } );
    var plan_ids =  table.cells( rows.nodes(), 1 ).data().toArray();
    var plan_numbers =  table.cells( rows.nodes(), 5 ).data().toArray();
