              ...
              <form class="col-sm-12" method="POST" id="bidders_form" action="{{route('submit_notice')}}">
                ...
                <div class="row">
                  <div class="form-group col-xs-6 col-sm-6 col-lg-6 mx-auto d-none">
                    ...
                    <label class="error-msg text-red" >@error('notice_id'){{$message}}@enderror</label>
                  </div>
                  ...
                  <div class="form-group col-xs-6 col-sm-6 col-lg-6 mb-0 d-none">
                    <label for="date_released">Date Released<span class="text-red">*</span></label>
                  ...
                  <div class="form-group col-xs-6 col-sm-6 col-lg-6 mb-0 d-none">
                    <label for="date_received">Date Received</span></label>
                    ...
                    <label class="error-msg text-red" >@error('date_received'){{$message}}@enderror</label>
                  </div>
                  ...
                  <div class="form-group col-xs-6 col-sm-6 col-lg-6 mb-0 d-none">
                    ...
                    <select type="text" id="notice_type" name="notice_type" class="form-control form-control-sm bg-white"  value="{{old('notice_type')}}" >
                      <option value="NTP"  {{ old('notice_type') == 'NTP' ? 'selected' : ''}}>Notice of Disqualification</option>
                    </select>
                    <label class="error-msg text-red" >@error('notice_type'){{$message}}@enderror</label>
                  </div>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td style="white-space: nowrap">
                    @if($project_plan->ntp_id!==null)
                    <button class="btn btn-sm btn btn-sm btn-primary  edit-ntp"> Edit NTP</button>
                    <a  class="btn btn-sm shadow-0 border-0 btn-primary text-white"  href="/generate_ntp/{{$project_plan->ntp_id}}"><i class="ni ni-cloud-download-95"></i></a>
                    @else
                    <button class="btn btn-sm btn btn-sm btn-success  edit-ntp" > Create NTP</button>
                    @endif
                  </td>
                  <td>{{ $project_plan->ntp_id }}</td>
                  ...
                  <td>{{ number_format($project_plan->proposed_bid,2,'.',',') }}</td>
                  <td>{{ $project_plan->ntp_release_date }}</td>
                  <td>{{ $project_plan->ntp_receive_date }}</td>
                  <td>{{ $project_plan->ntp_remarks }}</td>
...
if(oldInputs>2){
  ...
  if("@error('date_released') true @enderror"==' true ' || "@error('date_received') true @enderror"==' true '){
...
if("{{session('message')}}"){
  if("{{session('message')}}"=="range_error"){
    ...
    swal.fire({
      ...
      text: 'Sorry! You cannot update NTP, Please Input Contract Date Received.',
      buttonsStyling: false,
      confirmButtonClass: 'btn btn-sm btn-danger',
      icon: 'warning'
    });
...
$('#app_table tbody').on('click', '.edit-ntp', function (e) {
  ...
  if(data[1]=="" || data[1]==null){
    ...
    $("#notice_id").val('');
    $("#remarks").val('');
    $("#modal-title").html('Create Notice to Proceed');
  }
  else{
    $("#date_released").datepicker('setDate',moment(moment(data[14]).format('Y-MM-DD')).format("MM/DD/YYYY"));
    $("#date_received").datepicker('setDate',moment(moment(data[15]).format('Y-MM-DD')).format("MM/DD/YYYY"));
    $("#notice_id").val(data[1]);
    $("#remarks").val(data[16]);
    $("#modal-title").html('Edit Notice to Proceed');
  }
