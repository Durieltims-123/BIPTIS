        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h3 class="modal-title" id="form_modal_title">Set Approval By Higher Authority Date</h3>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div class="modal-body pt-0">
              <form class="col-sm-12" method="POST" id="submit_award_notice" action="/submit_authority_approval">
                @csrf
                <div class="row">
                  <div class="form-group col-xs-6 col-sm-6 col-lg-6 mx-auto">
                    <h5 for="authority_approval_date">Plan Id/s</h5>
                  ...
                  <div class="form-group col-xs-16 col-sm-6 col-md-6 mb-2  mb-0">
                    <h5 for="authority_approval_date">Approval By Higher Authority Date</h5>
                    <input type="text" id="authority_approval_date" name="authority_approval_date" class="form-control form-control-sm datepicker" value="{{old('authority_approval_date')}}" >
            ...
            <table class="table table-bordered" id="app_table">
              <thead class="">
                <tr class="bg-primary text-white" >
                  ...
                  <th class="text-center">Authority Approval</th>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="col-md-12" style="white-space: nowrap">
                    <button class="btn btn-sm btn-primary set_date_btn" >Set Date</button>

                    <a class="btn btn-sm btn-success" target='_blank' href="/project_bidders/{{$project_plan->plan_id}}" >Bidders</a>
                    @if($project_plan->re_bid_count<=3||$project_plan->re_bid_count==null)
                    <button class="btn btn-sm btn-warning rebid_btn" >Rebid</button>
                    @else
                    <button class="btn btn-sm btn-danger review_btn" >Review</button>
                    @endif
                  </td>
                  <td>{{ $project_plan->plan_id }}</td>
                  <td>{{ date("M d,Y", strtotime($project_plan->authority_approval_start))}} - {{ date("M d,Y", strtotime($project_plan->authority_approval_end))}}</td>
...
if(oldInputs>2){
  ...
  if('{{old("extend_plan_id")}}'!=''){
    ...
    $("#process").val("contract_preparation_signing");
  }
  if('{{old("plan_id")}}'!=''){
...
$("#extend_btn").click(function functionName() {
  if(table.rows( { selected: true } ).count()<1){
    ...
    $("#process").val("approval_by_higher_authority");
    $("#extend_days").val("");
    $("#extend_remarks").val("");
    $("#extend_modal").modal('show');
  }
