        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h3 class="modal-title" id="receive_modal_title">Receive Bid Doc </h3>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div class="modal-body">
              <form class="col-sm-12" method="POST" id="release_form" action="submit_receive_bid_doc">
                @csrf
                <div class="row d-flex">
                  ...
                  <!-- date released -->
                  <div class="form-group col-xs-6 col-sm-6 col-lg-6 ">
                    <label for="date_released">Date Bid Doc Released <span class="text-red">*</span>
                    </label>
...
if("{{session('message')}}"){
  if("{{session('message')}}"=="duplicate"){
    ...
    Swal.fire({
      ...
      text: 'Successfully deleted Bid Doc',
    ...
    swal.fire({
      title: `Error`,
      text: 'You cannot Update this Bid Doc',
      buttonsStyling: false,
      confirmButtonClass: 'btn btn-sm btn-danger',
      icon: 'warning'
    });
  ...
  columns: [
    ...
    render: function ( data, type, row ) {
      if(row.date_received==null){
        return '<div style="white-space: nowrap">@if(in_array("receive",$user_privilege))<button class="btn btn-sm btn btn-sm shadow-0 border-0 text-white btn-primary receive_btn" data-toggle="tooltip" data-placement="top" title="Receive" value="'+data+'"><i class="ni ni-box-2"></i></button> @endif</div>';
      }
      else{
        return '<div style="white-space: nowrap">@if(in_array("update",$user_privilege))<button class="btn btn-sm btn btn-sm shadow-0 border-0 text-white btn-success edit_btn" data-toggle="tooltip" data-placement="top" title="Edit" value="'+data+'"><i class="ni ni-ruler-pencil"></i></button> @endif</div>';
      }}
    },
    { "data": "bid_doc_id" },
...
$('#app_table tbody').on('click', '.edit_btn', function (e) {
  table.rows().deselect();
  $("#receive_modal_title").html("Receive Bid Doc");
