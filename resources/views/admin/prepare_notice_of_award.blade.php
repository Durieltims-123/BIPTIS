        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h3 class="modal-title" id="form_modal_title">Edit Notice of Award</h3>
                ...
                <div class="row">
                  ...
                  <div class="form-group col-xs-6 col-sm-6 col-lg-6 mb-0 d-none">
                    ...
                    <select type="text" id="notice_type" name="notice_type" class="form-control form-control-sm bg-white"  value="{{old('notice_type')}}" >
                      <option value="NOA"  {{ old('notice_type') == 'NOA' ? 'selected' : ''}}>Notice of Award</option>
                    </select>
      ...
      <div class="card shadow border-0">
        ...
        <div class="card-body">
          <div class="col-sm-12" id="filter">
            <form class="row" id="filter_notice" method="post" action="{{route('filter_prepare_notice_of_awards')}}">
              @csrf
...
var table=  $('#app_table').DataTable({
  ...
  data:data,
  columns: [
    { data:"notice_award_id",
    render: function ( data, type, row ) {
      if(data!=null){
        return '<div style="white-space: nowrap">@if(in_array("update",$user_privilege))<button class="btn btn-sm btn btn-sm btn-success edit-noa mr-0" data-toggle="tooltip" data-placement="top" title="Edit"> <i class="ni ni-ruler-pencil"></i></button>@endif <a  class="btn btn-sm btn btn-sm shadow-0 border-0 btn-primary text-white" target="_blank" data-toggle="tooltip" data-placement="top" title="View File"  href="/generate_noa/'+data+'"><i class="ni ni-cloud-download-95"></i></a></div>';
      }
      else{
        return '@if(in_array("add",$user_privilege))<div style="white-space: nowrap"><button class="btn btn-sm btn btn-sm btn-danger edit-noa mr-0" data-toggle="tooltip" data-placement="top" title="Add"> <i class="ni ni-fat-add"></i></button></div>@endif';
      }

    }},
    { "data": "notice_award_id" },
    { "data": "project_bid" },
    { "data": "award_notice_start" },
    { "data": "award_notice_end",render: function ( data, type, row ) {
      return moment(row.award_notice_start,'YYYY-MM-DD').format('MMMM DD,YYYY') +" - "+ moment(data,'YYYY-MM-DD').format('MMMM DD,YYYY');
    }},
...
$('#app_table tbody').on('click', '.edit-noa', function (e) {

  $('.error-msg').html("");
  var data = table.row( $(this).parents('tr') ).data();

  if(data.notice_award_id=="" || data.notice_award_id==null){
    $("#date_generated").val('');
    $("#date_released").val('');
    ...
    $("#notice_id").val('');
    $("#remarks").val('');
    $("#form_modal_title").html('Create Notice of Award');
  }
  else{
    $("#date_released").val('');
    $("#date_received_by_contractor").val('');
    $("#date_received_by_bac").val('');
    $("#date_generated").datepicker('setDate',moment(moment(data.date_generated).format('Y-MM-DD')).format("MM/DD/YYYY"));
    if(data.date_released!=null){
      $("#date_released").datepicker('setDate',moment(moment(data.date_released).format('Y-MM-DD')).format("MM/DD/YYYY"));
    }
    else{
      $("#date_released").val('');
    }
    ...
    if(data.date_received!=null){
      $("#date_received_by_bac").datepicker('setDate',moment(moment(data.date_received).format('Y-MM-DD')).format("MM/DD/YYYY"));
    }
    else{
      $("#date_received_by_bac").val('');
    }
    $("#notice_id").val(data.notice_award_id);
    $("#remarks").val(data.noa_remarks);
    $("#form_modal_title").html('Edit Notice of Award');
  }
