        ...
        <form class="col-sm-12" method="POST" id="resolution_form" action="{{route('submit_resolution')}}">
                ...
                <table class="table table-bordered" id="project_table">
                  <thead class="">
                    <tr class="bg-primary text-white" >
                      ...
                      @if($resolution_type==="RDF")<th class="text-center">Failure Status</th> @endif
                    </tr>
                  </thead>
                  <tbody>
                    ...
                    <tr>
                      ...
                      @if($resolution_type==="RDF")<td>{{$project_plan->failure_status}}</td> @endif
                    </tr>
                    @endforeach
                  </tbody>
                </table>
          ...
          <div class="form-group col-xs-6 col-sm-6 col-lg-6 mb-0 d-none" >
            ...
            <select type="text" id="resolution_type" name="resolution_type" class="form-control form-control-sm" >
              <option value="RRA" @if($resolution_type==="RRA") selected @endif>Resolution Recommending Approval</option>
              <option value="RDF" @if($resolution_type==="RDF") selected @endif>Resolution Declaring Failure</option>
...
function showOrHideReason() {
  ...
  var data=table.cells(rows, 9).data().sort().unique()
  if(rows.length==0){
    ...
    if(data.length>1){
      ...
      if(data[0]=="Failure After Post Qual"){
        $("#reason_container").addClass("d-none");
      }
      else{
        $("#reason_container").removeClass("d-none");
...
table.on( 'select', function ( e, dt, type, indexes ) {
  ...
  table.rows( { selected: false } ).every(function (rowIdx, tableLoop, rowLoop) {
    ...
    if(cluster_id!=null && cluster_id!="" && table.cell(rowIdx,8).data()==cluster_id){
      this.select();
    }
  }).order( [ 6, 'desc' ],[ 2, 'asc' ],[ 5, 'asc' ] ).draw();
