      ...
      <div class="card shadow border-0">
        ...
        <div class="card-body">
          <div class="col-sm-12 " id="filter">
            <form class="row" id="app_filter" method="POST" action="{{route('filter_twg_post_qual')}}">
              @csrf
              <!-- project year -->
...
<script>
  ...
  var table=  $('#app_table').DataTable({
    ...
    render: function ( data, type, row ) {
      let link='<div style="white-space: nowrap"><a class="btn btn-sm btn-success text-white" target="_blank" href="/twg_project_bidders/'+row.plan_id+'"><i class="ni ni-circle-08"></i></a>';
      if(row.twg_evaluation!=null &&  row.twg_evaluation!=''){
        link = link + ' <span class="btn btn-sm btn-success " >With PQER</span></div>';
      }
      else{
        link = link + ' <span class="btn btn-sm btn-info " >Without Evaluation</span></div>';
      }
      
      if(row.post_qual_days>row.maximum_days){
          link = link + ' <span class="btn btn-sm btn-danger " ><i class="ni ni-curved-next">Over Extension Date</i></span></div>';
        }
        else if(row.post_qual_days>=10 && row.post_qual_days<=12){
          link=link+' <span class="btn btn-sm btn-warning request_extension_btn" ><i class="ni ni-curved-next">Request Extension</i></span></div>';
        }
        else if(row.post_qual_days>=29 && row.post_qual_days<=30){
          link=link+'<span class="btn btn-sm btn-warning request_extension_btn" ><i class="ni ni-curved-next">Request Extension</i></span></div>';
        }
        else if(row.post_qual_days > 40){
          link=link+'<span class="btn btn-sm btn-danger">Due: '+moment(row.post_qualification_end,'Y-MM-DD').format('L')+'</span></div>';
        }
        else{
          link=link+'</div>';
        }
