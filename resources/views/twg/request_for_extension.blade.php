        ...
        var table = $('#data_table').DataTable({
            ...
            , buttons: [{
                ...
                , @if(in_array('add', $user_privilege)) {
                    text: 'Create Request For Extension'
                    , className: 'btn btn-sm shadow-0 border-0 bg-danger text-white'
                    , action: function(e, dt, node, config) {
                        var currentUrl = window.location.href;
                        currentUrl = currentUrl.replace('http://', '');
                        currentUrl = currentUrl.split('/');
                        window.open('http://' + currentUrl[0] + '/create_request_for_extension', '_blank');
                    }
                }
                , @endif {
                    text: 'Excel'
            ...
            , columns: [{
                    ...
                    , render: function(data, type, row) {
                        let edit = "/edit_request_for_extension/" + data;
                        let download = "/generate_request_for_extension/" + data;
                        let view = "/view_request_for_extension/" + data;

                        return "<div style='white-space: nowrap'>@if(in_array('update',$user_privilege))<a type='button' target='_blank' href='" + edit + "' class='btn btn-sm btn-success edit-btn' data-toggle='tooltip' data-placement='top' title='Edit' ><i class='ni ni-ruler-pencil'></i></a>@endif @if(in_array('delete',$user_privilege)) <button data-toggle='tooltip' data-placement='top' title='Delete'  class='btn btn-sm btn-danger delete-btn'><i class='ni ni-basket text-white'></i></button> @endif<a class='btn btn-sm bg-info text-white' data-toggle='tooltip' data-placement='top' title='Download'  target='_blank' href='" + download + "'><i class='ni ni-cloud-download-95'></i></a> <a type='button' target='_blank' href='" + view + "' class='btn btn-sm btn-info view-btn' data-toggle='tooltip' data-placement='top' title='View' ><i class='ni ni-tv-2'></i></a> </div>";
                    }
                }
                , {
                    "data": "request_id"
