      ...
      foreach ($plans as $plan) {
        ...
        if (in_array($plan->plan_id, $ids_array) == false) {
          ...
          if ($plan->procact_mode_id === 1) {
            ...
            $worksheet1->getCell('D15')->setValue(strtoupper(strtolower($plan->municipality_name)) . ', BENGUET');
            if ($plan->plan_cluster_id == null) {
            ...
            if (count($bidders) > 0) {
              ...
              $bidder_by_as_read = $APP->sortObject($array_bidders, array('proposed_bid' => 'asc'));
              $bidder_by_as_evaluated = $APP->sortObject($array_bidders, array('bid_as_evaluated' => 'asc'));
              $bidder_by_as_read = $bidder_by_as_read[0];
              $bidder_by_as_evaluated = $bidder_by_as_evaluated[0];
              $row1 = 62;
              $row2 = 62 + 9;
              if (count($bidder_by_as_read) > 1) {
                $row2 = $row1 + 8 + count($bidder_by_as_read);
                $worksheet1->insertNewRowBefore(63, (count($bidder_by_as_read) - 1));
                $worksheet1->insertNewRowBefore(($row2 + 1), (count($bidder_by_as_read) - 1));
              }

              foreach ($bidder_by_as_read as $value) {
              ...
              foreach ($bidder_by_as_evaluated as $value) {
          ...
          if (count($bidders) === 0) {
            ...
            foreach ($bidders as $bidder) {
              ...
              if ($bidder->lowest != null && $bidder->bid_status != "disqualified" && $bidder->bid_status != "ineligible") {
                $lowest_style = [
    ...
    if (count($plans) > 0) {
      ...
      foreach ($plans as $plan) {
        if (in_array($plan->plan_id, $ids_array) == false) {
          ...
          if ($same_location === true) {
            if ($plan->barangay_id != null) {
              $location = htmlspecialchars(strtoupper(strtolower($plan->municipality_name)));
            } else {
